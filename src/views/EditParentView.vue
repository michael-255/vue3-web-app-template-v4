...
onMounted(async () => {
  try {
    if (await idValidator.isValid(routeId)) {
      const editRecord = (await DB.getParent(routeId as string)) as any
    ...
    log.error('Error loading edit parent view', error)
  }
})
...
async function onSubmit() {
  confirmDialog('Update', `Update ${labelSingular} record?`, Icon.SAVE, 'positive', async () => {
    try {
      ...
      await DB.updateParent(routeId as string, deepRecordCopy)
